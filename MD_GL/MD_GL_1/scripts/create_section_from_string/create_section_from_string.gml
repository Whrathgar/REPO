...
for (c = 1; c < string_length(_section_string_data) + 1; c++)
{
	switch (string_char_at(_section_string_data, c))
	{
		...
		case "p":
			...
			{
				instance_create_layer((_current_x_section * section_width) + (((c) mod section_tiles_x) * grid_size) + (1.5 * grid_size), 
										(_current_y_section * section_height) + (floor ((c) / section_tiles_x) * grid_size) + (1.5 * grid_size),
										"Instances", o_player);
			}
			break;
	}
